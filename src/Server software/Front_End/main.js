...
function createAllDevices(dev_array) {
	...
	dev_array.forEach(function (device) {
		...
		html_dev_obj += "<h4 style='text-decoration:underline;color:yellow'>Basic device info:</h4>";
		html_dev_obj += "<span class='destate'>Current state: : " + state + "</span><br>";
		html_dev_obj += "<span class='dev_id_s'>Device ID : " + devid + "</span>";
		html_dev_obj += "</div>";
		html_dev_obj += "<div class='mogucnosti text-center'>";
		html_dev_obj += "<h4 style='text-decoration:underline;color:yellow'>Control</h4>";
...
function getDeviceOptions(device) {
	...
	switch (dev_type) {
	case 0:
		ret = "<button onclick='switchDeviceState(uid,passwd," + devid + ",1)' class='btn btn-default btn-flat btn-success'>Turn device off</button>";
		ret += "<button onclick='switchDeviceState(uid,passwd," + devid + ",0)' class='btn btn-default btn-flat btn-danger'>Turn device on</button>";
		return ret;
	case 1:
	    ret = "<span class='text-info'>Enter PWM value from 0 to 255 : </span><br>";
		ret += "<input devid = "+devid+" type='text' class='pwm_input'></input><br><button class='btn btn-default btn-flat pwm_button btn-info'>Apply</button>";
		return ret;
	case 2:
		ret +="<input class='devid_holder' devid='"+devid+"' id='devid-"+devid+"' type='hidden' value='"+state+"'/>";
		ret +="<button class=\"btn btn-default btn-flat jscolor {valueElement:'devid-"+devid+"'}\" value='"+state+"'>Choose color</button><br>";
		ret +="<button class='btn btn-default btn-flat color_button btn-info'>Apply</button>";
		return ret;
		break;
	case 3:
		ret = "You can't operate this device";
		return ret;
		break;
