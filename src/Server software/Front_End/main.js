...
function switchDeviceState(uid,password,devid,state){
            var query = "promjenistanje:"+uid.toString()+":"+password.toString()+":"+devid.toString()+":"+state.toString();
            $.post( "el_interface.php", { uid: uid, q: query }).done(function(data) {
                    var d = data.toString();
                   console.log(d)
                   }).fail(function(){
                    console.log("NIJE MOGUCE USPOSTAVITI VEZU SA SERVEROM");
                 });
}
    
function deviceChanged(dev_id,dev){
 if($('.'+dev_id.toString()).length==0){
  console.log("ne postoji uredjaj");
 }else{
 dev.forEach(function(device,index){
     if(device[0] == dev_id){
								  ...
								  html_dev_obj += "</div>";
								  html_dev_obj += "</div>";
								  $('.'+dev_id.toString()).replaceWith(html_dev_obj); 
                                                                  glob_devices[index] = dev[index]
     }
});
